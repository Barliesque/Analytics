...
namespace Barliesque.Analytics
{
	...
	public class Analytics : MonoBehaviour
	{
		...
		static private Analytics FindInstance(string tableID)
		{
			...
			{
				var equal = string.Compare(inst._tableID, tableID, CultureInfo.InvariantCulture,
		...
		static private Analytics GetInstance()
		{
			if (_instance == null || _instance.Count == 0)
				throw new Exception("[ANALYTICS]  There are no active Analytics instances, or it has not yet initialized");
			if (_instance.Count > 1)
				throw new Exception("[ANALYTICS]  Analytics tableID must be specified when multiple Analytics components are in use!");
			return _instance[0];
		}
		...
		private void SetGameStarted(string timestamp)
		{
			...
			if (!string.IsNullOrEmpty(timestamp))
			{
				...
				if (metric.type != AnalyticsMetric.ValueType._dateTime)
					Debug.LogError($"[ANALYTICS]  Timestamp metric \"{timestamp}\" must have type Timestamp.");
		...
		private void SetGameEnded(string duration)
		{
			...
			if (!string.IsNullOrEmpty(duration))
			{
				...
				if (metric.type != AnalyticsMetric.ValueType._timeSpan)
					Debug.LogError($"[ANALYTICS]  Duration metric \"{duration}\" must have type Timespan.");
